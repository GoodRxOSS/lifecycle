# E2B workspace template (operator workflow)

The E2B workspace backend boots sandboxes from a prebuilt E2B *template* — a Firecracker snapshot
built from the Lifecycle workspace gateway plus the launcher in this directory. Building the
template is an **operator step** done with the `e2b` CLI; it is not part of the Lifecycle runtime or
