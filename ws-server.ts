...
async function lookupChatPreviewEndpoint(match: ChatPreviewPathMatch): Promise<RemoteEndpointRef> {
  ...
  const { parsePersistedPreviewEndpoint } = await import('./src/server/services/workspaceRuntime/gatewayPreview');
  if (match.previewSlug) {
    ...
    return parsePersistedPreviewEndpoint(exposure.providerState || {});
  }
