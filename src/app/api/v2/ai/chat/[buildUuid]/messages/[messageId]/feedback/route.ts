...
/**
 * @openapi
 * /api/v2/ai/chat/{buildUuid}/messages/{messageId}/feedback:
 *   post:
 *     summary: Submit message feedback
 *     description: >
 *       Submit a thumbs up/down rating for a specific agent message.
 ...
 *         schema:
 *           type: integer
 *         description: The ID of the message to rate.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *                 type: integer
 *                 description: >
 *                   Optional assistant message timestamp. Use this when a persisted
 *                   message ID is not yet available.
 *     responses:
 *       '201':
 ...
 */
const postHandler = async (req: NextRequest, { params }: { params: { buildUuid: string; messageId: string } }) => {
  const { buildUuid, messageId } = params;
  ...
  if (isNaN(parsedMessageId) || parsedMessageId < 0) {
    return errorResponse(new Error('Invalid messageId: must be a non-negative integer'), { status: 400 }, req);
  }

  const body = await req.json();
  const { rating, text, messageTimestamp } = body;
  ...
  let parsedMessageTimestamp: number | undefined;
  if (messageTimestamp !== undefined) {
    if (typeof messageTimestamp === 'number' && Number.isFinite(messageTimestamp)) {
      parsedMessageTimestamp = messageTimestamp;
    } else if (typeof messageTimestamp === 'string') {
      const value = Number(messageTimestamp);
      if (Number.isFinite(value)) {
        parsedMessageTimestamp = value;
      }
    }

    if (parsedMessageTimestamp === undefined) {
      return errorResponse(new Error('Invalid messageTimestamp: must be a number'), { status: 400 }, req);
    }
  }

  const resolvedMessageId = parsedMessageId > 0 ? parsedMessageId : undefined;
  if (!resolvedMessageId && parsedMessageTimestamp === undefined) {
    return errorResponse(
      new Error('Either a valid messageId path param or messageTimestamp is required'),
      { status: 400 },
      req
    );
  }
  ...
  try {
    const record = await feedbackService.submitMessageFeedback({
      buildUuid,
      messageId: resolvedMessageId,
      messageTimestamp: parsedMessageTimestamp,
      rating,
      text,
      userIdentifier,
      repo,
