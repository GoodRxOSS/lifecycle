...
const putHandler = async (req: NextRequest, { params }: { params: Promise<{ threadId: string }> }) => {
  ...
  // SECURITY: git_write can never be auto-approved; reject instead of storing a silently inert entry.
  const ineligible = toolKeys.filter((toolKey) => !ApprovalService.isToolKeyAlwaysAllowEligible(toolKey));
  if (ineligible.length > 0) {
    return errorResponse(
      new Error(`These tools always require an explicit approval and cannot be allowlisted: ${ineligible.join(', ')}`),
      { status: 400 },
      req
    );
  }
