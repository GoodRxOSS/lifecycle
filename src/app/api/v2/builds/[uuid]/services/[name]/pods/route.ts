...
import { getDeploymentPods } from 'server/lib/kubernetes/getDeploymentPods';

/**
 * @openapi
 * /api/v2/builds/{uuid}/services/{name}/pods:
 *   get:
 *     summary: List pods for a service
 *     description: |
 *       Returns a list of all pods for a specific service within a build.
 *       This includes pod status, age, restarts, readiness, and container information.
 *     tags:
 *       - Deployments
 *     operationId: listDeploymentPods
 *     parameters:
 *       - in: path
 *         name: uuid
 *         required: true
 ...
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/GetDeploymentPodsSuccessResponse'
 *       '400':
...
const getHandler = async (req: NextRequest, { params }: { params: { uuid: string; name: string } }) => {
  ...
  try {
    const pods = await getDeploymentPods(name, uuid);

    const response = { pods };

    return successResponse(response, { status: 200 }, req);
  } catch (error) {
    getLogger().error({ error }, `API: pods fetch failed service=${name}`);
