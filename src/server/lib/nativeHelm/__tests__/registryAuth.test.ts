...
describe('registryAuth', () => {
  describe('detectRegistryAuth', () => {
    it('returns ECR config for a valid ECR OCI URL', () => {
      const result = detectRegistryAuth('oci://441168312025.dkr.ecr.us-west-2.amazonaws.com/my-chart');
    ...
    it('returns undefined for a non-ECR OCI URL', () => {
      const result = detectRegistryAuth('oci://registry-1.docker.io/bitnamicharts/postgresql');
      expect(result).toBeUndefined();
    });

    it('returns undefined for a non-OCI URL', () => {
      const result = detectRegistryAuth('https://charts.bitnami.com/bitnami');
      expect(result).toBeUndefined();
    });
