...
import { McpToolAnnotations, ResolvedMcpServer, MCP_ERROR_CODES, McpDiscoveredTool } from './types';
...
export class MCPToolAdapter implements Tool {
  ...
  private serverTransport: ResolvedMcpServer['transport'];
  private serverDefaultArgs: Record<string, string>;
  private serverTimeout: number;

  constructor(
    serverId: string,
    discoveredTool: McpDiscoveredTool,
    serverTransport: ResolvedMcpServer['transport'],
    serverDefaultArgs: Record<string, string>,
    serverTimeout: number
  ) {
    this.name = `mcp__${serverId}__${discoveredTool.name}`;
    this.safetyLevel = mapAnnotationsToSafetyLevel(discoveredTool.annotations);
    this.description = prefixDescription(
      discoveredTool.description || `MCP tool ${discoveredTool.name} from ${serverId}`,
      this.safetyLevel
    );
    this.parameters = discoveredTool.inputSchema as JSONSchema;
    this.category = 'mcp';
    this.originalName = discoveredTool.name;
    this.serverTransport = serverTransport;
    this.serverDefaultArgs = serverDefaultArgs;
    this.serverTimeout = serverTimeout;
  }

  async execute(args: Record<string, unknown>, signal?: AbortSignal): Promise<ToolResult> {
    const resolvedArgs = applyMcpDefaultToolArgs(
      this.parameters as Record<string, unknown>,
      this.serverDefaultArgs,
      args
    );
    const client = new McpClientManager();
    try {
      try {
        await client.connect(this.serverTransport, this.serverTimeout);
      } catch (error) {
        getLogger().warn(
      ...
      const result = await client.callTool(this.originalName, resolvedArgs, this.serverTimeout, signal);
...
export function createMcpTools(servers: ResolvedMcpServer[]): Tool[] {
  ...
  for (const server of servers) {
    for (const discoveredTool of server.discoveredTools) {
      tools.push(new MCPToolAdapter(server.slug, discoveredTool, server.transport, server.defaultArgs, server.timeout));
    }
  }
  return tools;
}
