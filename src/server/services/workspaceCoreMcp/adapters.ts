...
function payloadHasPolicyCode(payload: unknown): boolean {
  if (!isRecord(payload) || payload.ok !== false) {
    return false;
  }
  const code = readString(payload.code);
  return code === 'capability_required' || isToolPolicyErrorCode(code);
}
...
/** Uncoded/non-policy-coded gateway failures (capacity, spawn errors) must keep their reason visible to the model. */
function mapExecFailureResult(payload: Record<string, unknown>) {
  const result = mapCommandResult(payload);
  const reason = [readString(payload.code), readString(payload.error), readString(payload.details)]
    .filter(Boolean)
    .join(': ');

  return {
    ...result,
    status: result.status === 'timed_out' || result.status === 'cancelled' ? result.status : 'failed',
    stderr:
      [result.stderr, reason].filter(Boolean).join('\n') || 'workspace_core.exec failed in the workspace runtime.',
  };
}
...
async function executeGatewayAdapter(
  ...
  // For exec, an ordinary non-zero exit is returned as ok:false WITHOUT a policy code; it must fall
  // through to mapCommandResult (status/exit_code/stdout/stderr) instead of being masked as an
  // invalid_arguments policy error, which would strip the command output the agent needs.
  const isExec = definition.name === 'exec';
  if (!isExec || payloadHasPolicyCode(payload)) {
  ...
  const failure = isExec ? null : mapGatewayFailure(definition.name, payload);
  if (failure) {
    return failure;
  }
...
async function executeExec(input: Record<string, unknown>, context: WorkspaceCoreAdapterContext) {
  ...
  if (isRecord(payload) && payload.ok === false) {
    // Policy-coded failures were already mapped upstream; everything else keeps its reason in schema shape.
    if (payloadHasPolicyCode(payload)) {
      return payload;
    }

    return mapExecFailureResult(payload);
  }

  const result = mapCommandResult(payload);
  // Handle-mode (async) failures arrive ok:true with the reason only in the snapshot's error field.
  if (isRecord(payload) && (result.status === 'failed' || result.status === 'timed_out') && readString(payload.error)) {
    return mapExecFailureResult(payload);
  }
...
function mapServiceSnapshot(payload: unknown) {
  const value = isRecord(payload) ? payload : {};
  return {
    service_name: readPropertyString(value, ['name', 'serviceName']),
    status: readString(value.status),
    running: readPropertyBoolean(value, ['running']) ?? false,
    ...(readNumber(value.pid) !== undefined ? { pid: readNumber(value.pid) } : {}),
    ...(readNumber(value.port) !== undefined ? { port: readNumber(value.port) } : {}),
    ...(readPropertyString(value, ['startedAt', 'started_at'])
      ? { started_at: readPropertyString(value, ['startedAt', 'started_at']) }
      : {}),
    ...(readNumber(value.exitCode) !== undefined ? { exit_code: readNumber(value.exitCode) } : {}),
    ...(readPropertyString(value, ['stdout']) !== undefined ? { stdout: readPropertyString(value, ['stdout']) } : {}),
    ...(readPropertyString(value, ['stderr']) !== undefined ? { stderr: readPropertyString(value, ['stderr']) } : {}),
    ...(readPropertyString(value, ['error']) ? { error: readPropertyString(value, ['error']) } : {}),
  };
}

async function executeStartService(input: Record<string, unknown>, context: WorkspaceCoreAdapterContext) {
  const command = readString(input.command);
  if (!command) {
    return invalidArgumentsResult('start_service', 'command must be a non-empty string.');
  }
  ...
  const payload = await executeGatewayAdapter(
    ...
    {
      command,
      ...(readString(input.service_name) ? { serviceName: readString(input.service_name) } : {}),
      ...(readString(input.cwd) ? { cwd: readString(input.cwd) } : {}),
      ...(readNumber(input.port) !== undefined ? { port: readNumber(input.port) } : {}),
      ...(typeof input.restart === 'boolean' ? { restart: input.restart } : {}),
      ...(readNumber(input.wait_ms) !== undefined ? { waitMs: readNumber(input.wait_ms) } : {}),
    },
    context
  );
  ...
  return mapServiceSnapshot(payload);
}

async function executeServiceStatus(input: Record<string, unknown>, context: WorkspaceCoreAdapterContext) {
  const payload = await executeGatewayAdapter(
    {
      name: 'service_status',
    ...
    {
      ...(readString(input.service_name) ? { serviceName: readString(input.service_name) } : {}),
      ...(typeof input.include_logs === 'boolean' ? { includeLogs: input.include_logs } : {}),
      ...(readNumber(input.max_chars) !== undefined ? { maxChars: readNumber(input.max_chars) } : {}),
    },
    context
  );
  if (isRecord(payload) && payload.ok === false && payload.code) {
...
export async function executeWorkspaceCoreTool(
  ...
  switch (definition.name) {
    ...
    case 'start_service':
      return executeStartService(input, context);
    case 'service_status':
      return executeServiceStatus(input, context);
    case 'read_file':
