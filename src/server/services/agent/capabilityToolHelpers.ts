...
export function buildAiToolApprovalConfig(
  toolApproval: AgentRuntimeToolApprovalConfig | undefined,
  options?: { autoApprovedToolKeys?: ReadonlySet<string> }
  ...
  const approval: ToolApprovalConfiguration<ToolSet, AgentRuntimeContext> = async ({
    ...
    if (options?.autoApprovedToolKeys?.has(toolCall.toolName)) {
      return { type: 'approved', reason: 'Allowed for this conversation by the user' };
    }
