    ...
    const tools = await AgentCapabilityService.buildToolSet({
      ...
      hooks: {
        onToolStarted: async (audit) => {
          const pendingAction = audit.toolCallId
            ? await AgentPendingAction.query()
                .where({ threadId: thread.id, runId: run.id })
        ...
        onToolFinished: async (audit) => {
          const executionQuery = AgentToolExecution.query().where({ runId: run.id });
    ...
    const agent = new ToolLoopAgent({
      ...
      onStepFinish: async (step) => {
        try {
          ...
          getLogger().warn({ error, runId: run.uuid }, `AgentExec: step observability patch failed runId=${run.uuid}`);
        }
      },
      onFinish: (event) => {
      ...
      onStreamFinish: async ({
        ...
        if (finishReason === 'error') {
          await AgentRunService.markFailed(
            run.uuid,
            new Error('Agent stream finished with error'),
