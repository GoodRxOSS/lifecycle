...
import AgentMessageStore, { type RuntimeControlsUpdateChoice } from './MessageStore';
...
function runtimeControlsActorLabel(userIdentity: RequestUserIdentity): string {
  return (
...
function selectedChoiceLabels(state: AgentThreadRuntimeControlsState): Map<string, string> {
  const selected = new Map<string, string>();
  for (const choice of [...state.tools.required, ...state.tools.optional, ...state.mcp.connections]) {
    if (choice.selected) {
      selected.set(choice.id, choice.label);
    }
  }
  return selected;
}

function diffSelectedChoices(
  before: AgentThreadRuntimeControlsState,
  after: AgentThreadRuntimeControlsState
): { enabled: RuntimeControlsUpdateChoice[]; disabled: RuntimeControlsUpdateChoice[] } {
  const beforeSelected = selectedChoiceLabels(before);
  const afterSelected = selectedChoiceLabels(after);
  const enabled = [...afterSelected].filter(([id]) => !beforeSelected.has(id)).map(([id, label]) => ({ id, label }));
  const disabled = [...beforeSelected].filter(([id]) => !afterSelected.has(id)).map(([id, label]) => ({ id, label }));
...
export default class AgentThreadRuntimeControlsService {
  ...
  static async patchChoices({
    ...
    const nextState = buildChoiceState({
      ...context,
      savedChoices: validatedMetadata,
    }).state;

    // The model only learns the tool surface from history; record the change where future runs read it.
    // Informational only — enforcement stays in the run-plan snapshot and tool registration.
    const { enabled, disabled } = diffSelectedChoices(state, nextState);
    if (enabled.length > 0 || disabled.length > 0) {
      await AgentMessageStore.createRuntimeControlsUpdateEvent({
        thread: { id: context.threadRecordId },
        actor: { userId: userIdentity.userId, label: runtimeControlsActorLabel(userIdentity) },
        enabled,
        disabled,
      }).catch((error) => {
        getLogger().warn(
          { error, threadId },
          `AgentThread: runtime-controls update event append failed threadId=${threadId}`
        );
        return null;
      });
    }
