...
describe('tool_call input round-trip idempotency (per-run history rewrite)', () => {
  const LONG_INPUT = JSON.stringify({ new_content: 'x'.repeat(5_000), path: 'a.txt' });

  it('re-persisting a replayed over-long tool input does not nest escaped preview wrappers', () => {
    ...
    // Simulate the per-run-finish rewrite: canonical -> UI -> canonical, repeated.
    let round = toUiMessageFromCanonicalInput({ id: 'm', role: 'assistant', parts: canonicalParts });
    const first = getCanonicalPartsFromUiMessage(round);
    round = toUiMessageFromCanonicalInput({ id: 'm', role: 'assistant', parts: first });
    const second = getCanonicalPartsFromUiMessage(round);
    round = toUiMessageFromCanonicalInput({ id: 'm', role: 'assistant', parts: second });
    const third = getCanonicalPartsFromUiMessage(round);

    expect(second).toEqual(first);
    expect(third).toEqual(first);
    const input = (first[0] as { input: string }).input;
    expect(input).not.toContain('\\"preview\\"');
  });
});
