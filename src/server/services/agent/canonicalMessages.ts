...
function toolValueToText(value: unknown): string | null {
  ...
  // Unwrap the { preview } replay sentinel so re-persisting stays idempotent.
  if (isPreviewWrapper(value)) {
...
function isPreviewWrapper(value: unknown): value is { preview: string } {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const keys = Object.keys(value);
  return keys.length === 1 && keys[0] === 'preview' && typeof (value as { preview: unknown }).preview === 'string';
}
  ...
  switch (part.type) {
    ...
    case 'source_ref': {
      ...
      return {
        ...
        sourceId: normalizeText(part.sourceId),
        mediaType: normalizeText(part.mediaType),
      };
    }
    case 'tool_call': {
...
export function toUiMessageFromCanonicalInput(
  ...
  for (const part of message.parts) {
    ...
    if (part.type === 'file_ref') {
      // ai@7 file parts require url+mediaType; project path-only refs to a valid source-document instead.
      if (part.url) {
        parts.push({
          type: 'file',
          url: part.url,
          mediaType: part.mediaType || 'application/octet-stream',
          ...(part.title ? { filename: part.title } : {}),
      ...
      parts.push({
        ...
        ...(part.state === 'completed'
          ...
          : {
              state: 'output-denied',
              input,
              // ai@7 requires approval:{id, approved:false} on denied parts; synthesize a missing id.
              approval: {
                id: part.approval?.id || uuid(),
                approved: false,
    ...
    // ai@7 requires sourceId (+mediaType/title on documents); always emit a valid shape.
    if (part.url) {
      parts.push({
        type: 'source-url',
        sourceId: part.sourceId || uuid(),
        url: part.url,
