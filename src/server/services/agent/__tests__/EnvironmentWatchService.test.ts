...
import { repairObservationExpectsRebuild, REPAIR_OBSERVATION_NO_REBUILD_TEXT } from '../debugRepairObservation';
import EnvironmentWatchService, {
  buildEnvironmentWatchMessageText,
...
const mockUpsert = AgentMessageStore.upsertCanonicalUiMessagesForThread as jest.Mock;
...
describe('buildEnvironmentWatchMessageText', () => {
  it('formats the success message', () => {
    expect(buildEnvironmentWatchMessageText({ outcome: 'success' })).toBe(
      'Environment rebuilt successfully after the repair: status=deployed.'
    );
  });

  it('formats the timeout message with the current status', () => {
    expect(buildEnvironmentWatchMessageText({ outcome: 'timeout', status: 'building' })).toBe(
      'The rebuild has not reached a terminal state after 30 minutes (status=building).'
    );
  });

  it('formats the failure message with capped failing services', () => {
    const failingDeploy = (name: string) => ({
      uuid: `${name}-uuid`,
      status: DeployStatus.DEPLOY_FAILED,
      statusMessage: `${name} crashed`,
      deployable: { name },
      service: null,
    });
    const text = buildEnvironmentWatchMessageText({
      ...
      deploys: [failingDeploy('one'), failingDeploy('two'), failingDeploy('three'), failingDeploy('four')],
    });

    expect(text).toContain('The rebuild after the repair finished with status=error, message=Deployment failed.');
    expect(text).toContain('Current blocker: one status=deploy_failed, message=one crashed');
    expect(text).toContain('three status=deploy_failed');
    expect(text).not.toContain('four status=deploy_failed');
  });

  it('falls back to a generic blocker hint when no deploy is failing', () => {
    const text = buildEnvironmentWatchMessageText({ outcome: 'failure', status: BuildStatus.ERROR, deploys: [] });
    expect(text).toBe(
      'The rebuild after the repair finished with status=error. Check the latest deploy details for the current blocker.'
    );
  });
});

describe('repairObservationExpectsRebuild', () => {
  it('expects no rebuild for empty or no-commit observations', () => {
    expect(repairObservationExpectsRebuild(null)).toBe(false);
    expect(repairObservationExpectsRebuild(REPAIR_OBSERVATION_NO_REBUILD_TEXT)).toBe(false);
  });

  it('expects no rebuild when the observation already reports a terminal outcome', () => {
    expect(
      repairObservationExpectsRebuild(
        'Commit: x. Fresh Lifecycle state: Lifecycle picked up the repair commit and the environment is deployed.'
      )
    ).toBe(false);
    expect(
      repairObservationExpectsRebuild(
        'Fresh Lifecycle state: Lifecycle picked up the repair commit, but the environment is still terminal status=error.'
      )
    ).toBe(false);
  });

  it('expects a rebuild for in-progress and not-yet-observed observations', () => {
    expect(
      repairObservationExpectsRebuild(
        'Fresh Lifecycle state: Lifecycle picked up the repair commit and the environment is still in progress with status=building.'
      )
    ).toBe(true);
    expect(
      repairObservationExpectsRebuild(
        'Fresh Lifecycle state: the repair commit has not shown up on this environment yet, so a webhook rebuild has not been observed. Current environment status=error.'
      )
    ).toBe(true);
  });
});
...
describe('processWatchJob', () => {
  beforeEach(() => {
    ...
    redis.del.mockResolvedValue(1);
    (AgentThread.query as jest.Mock).mockReturnValue({
      findOne: jest.fn().mockResolvedValue({ id: 5, uuid: 'thread-1' }),
    });
  });
  ...
  it('re-enqueues a delayed poll while the rebuild is in progress', async () => {
    ...
    expect(payload).toMatchObject({ pollCount: 1, sawActivity: true });
  ...
  it('appends a success system message and releases the marker once deployed', async () => {
    mockBuildLoad({ status: BuildStatus.DEPLOYED, statusMessage: null, updatedAt: 't2', deploys: [] });

    await EnvironmentWatchService.processWatchJob(watchJob({ sawActivity: true, pollCount: 3 }));

    expect(mockQueueAdd).not.toHaveBeenCalled();
    expect(mockUpsert).toHaveBeenCalledTimes(1);
    const [thread, messages] = mockUpsert.mock.calls[0];
    expect(thread).toEqual({ id: 5 });
    expect(messages).toHaveLength(1);
    expect(messages[0]).toMatchObject({
      id: 'watch-1',
      role: 'system',
      parts: [{ type: 'text', text: 'Environment rebuilt successfully after the repair: status=deployed.' }],
    });
  ...
  it('appends a failure message with the failing services', async () => {
    mockBuildLoad({
      status: BuildStatus.ERROR,
      statusMessage: 'Deployment failed',
      updatedAt: 't2',
    ...
    expect(messages[0].parts[0].text).toContain(
      'The rebuild after the repair finished with status=error, message=Deployment failed.'
    );
    expect(messages[0].parts[0].text).toContain('Current blocker: web status=deploy_failed, message=CrashLoopBackOff');
    expect(messages[0].metadata.outcome).toBe('failure');
  });
  ...
  it('reports a timeout when the deadline passes without a terminal status', async () => {
    ...
    expect(messages[0].metadata.outcome).toBe('timeout');
