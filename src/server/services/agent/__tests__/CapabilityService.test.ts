...
describe('AgentCapabilityService.buildToolSet', () => {
  ...
  it('rejects ready chat runtimes whose gateway does not satisfy the workspace contract', async () => {
    mockResolveServers.mockResolvedValue([]);
    ...
    await expect(
      buildToolSetWithMetadataForTest({
        session: {
          uuid: 'session-chat',
          sessionKind: 'chat',
          workspaceStatus: 'ready',
    ...
    ).rejects.toThrow(/Workspace gateway contract v\d+ is not satisfied.*workspace.service_start/);
