...
describe('AgentRunPlanResolver', () => {
  ...
  it('resolves explicit redeploy requests as repair after a completed diagnosis', async () => {
    const findPriorCompletedDebugIntentRun = jest.fn().mockResolvedValue(true);
    ...
    const result = await resolve({
      ...
      messageText: 'The config was corrected. Trigger a redeploy of this environment now and watch the result.',
      findPriorCompletedDebugIntentRun,
    });
