...
describe('repairAgentToolName', () => {
  ...
  it('strips a chat/session slug prefix the model prepends (the observed a6534157 failure)', () => {
    expect(repairAgentToolName('chat_a6534157__mcp__workspace_core__list_files', KEYS)).toBe(
      'mcp__workspace_core__list_files'
    );
    expect(repairAgentToolName('chat-a6534157__mcp__workspace_core__list_files', KEYS)).toBe(
      'mcp__workspace_core__list_files'
    );
  });
