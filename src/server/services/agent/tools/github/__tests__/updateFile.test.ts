...
describe('UpdateFileTool', () => {
  ...
  it('fails closed when an approved write has no user GitHub auth', async () => {
    ...
    mockGithubClient.getOctokitWithAuth
      .mockResolvedValueOnce({ octokit: mockOctokit, auth: { provider: 'github', source: 'app', required: false } })
      .mockRejectedValueOnce(new GitHubUserAuthRequiredError({ provider: 'github', source: 'none', required: true }));
  ...
  it('returns the friendly no-op result without requiring write authorization', async () => {
    mockOctokit.request.mockResolvedValueOnce({
      data: { sha: 'existing-sha', content: Buffer.from(baseArgs.new_content).toString('base64') },
    });
    // No write auth on this fresh run — a no-op must not dead-end on 'Reconnect GitHub'.
    mockGithubClient.getOctokitWithAuth.mockImplementation(
      async (_caller: string, options: { requireUserAuth: boolean }) => {
        if (options.requireUserAuth) {
          throw new GitHubUserAuthRequiredError({ provider: 'github', source: 'none', required: true });
        }
        return { octokit: mockOctokit, auth: { provider: 'github', source: 'app', required: false } };
      }
    );
    ...
    expect(JSON.parse(result.agentContent)).toMatchObject({ changed: false, commit_created: false });
    expect(mockOctokit.request).toHaveBeenCalledTimes(1);
  });
  ...
  it('rejects an invalid lifecycle.yaml without committing', async () => {
    ...
    // Validation rejects before the write-auth handoff is ever demanded.
    expect(mockGithubClient.getOctokitWithAuth).toHaveBeenCalledWith('agent-runtime-update-file', {
      requireUserAuth: false,
      toolCallId: 'tool-update-file',
    });
    expect(mockGithubClient.getOctokitWithAuth).not.toHaveBeenCalledWith(
      'agent-runtime-update-file',
      expect.objectContaining({ requireUserAuth: true })
    );
    expect(mockOctokit.request).toHaveBeenCalledTimes(1);
  });
