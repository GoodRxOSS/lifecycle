...
export const DEBUG_INSTRUCTION_TEMPLATE_DEFAULT_CONTENT = [
  ...
  '- Build- or deploy-phase failure: if the Deploy has a buildPipelineId (build) or deployPipelineId (deploy) in the DEPLOYS section, use get_codefresh_logs with that id. Its default view is a truncated tail; when the failure is not visible there, re-call with search="<regex>" (scans the entire log server-side, e.g. "error|failed|exit code" or a specific signature) and read around a hit with start_line — do not conclude from the truncated view alone. Otherwise get_build_logs(service_name) returns the persisted log (same search parameter) and falls back to the live job pod; if it reports nothing, the job pods were likely garbage-collected — check get_k8s_resources events instead. Do not re-fetch buildOutput via query_database; get_build_logs already read it.',
  ...
  '- When output is truncated, do not assume the omitted region is error-free; if your conclusion depends on it, re-fetch with a narrower query: search="<regex>" on get_codefresh_logs/get_build_logs/get_pod_logs scans the full log server-side (get_codefresh_logs also pages with start_line); for k8s resources use label_selector or a single pod.',
