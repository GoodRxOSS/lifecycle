...
import { IN_PROGRESS_BUILD_STATUSES } from './debugRepairObservation';
...
export type AgentEnvironmentWatchJob = {
  ...
  sawActivity?: boolean;
  activityEventPosted?: boolean;
  commitUrl?: string | null;
  pollCount: number;
  deadlineAt: string;
  // Carried so any early-exit (including an invalid-payload reject) can release the dedupe
  // marker instead of blocking re-arms for the full marker TTL.
...
export function buildEnvironmentWatchHeadline(
  outcome: 'started' | 'success' | 'failure' | 'timeout',
  reason: EnvironmentWatchReason
): string {
  const cause = reason === 'repair_commit' ? 'after the repair commit' : 'after the redeploy trigger';
  switch (outcome) {
    case 'started':
      return `Rebuild started ${cause}.`;
    case 'success':
      return `Rebuild ${cause} finished: environment deployed.`;
    case 'failure':
      return `Rebuild ${cause} finished with a failure.`;
    case 'timeout':
      return `Rebuild ${cause} has not reached a terminal state after 30 minutes.`;
  }
}
...
export default class EnvironmentWatchService {
  ...
  static async scheduleEnvironmentWatch(input: ScheduleEnvironmentWatchInput): Promise<ScheduleEnvironmentWatchResult> {
    try {
      ...
      const payload: AgentEnvironmentWatchJob = {
        // Context first: it carries ambient buildUuid/etc. that must not clobber the watch fields.
  ...
  static async processWatchJob(job: Job<AgentEnvironmentWatchJob>): Promise<void> {
    ...
    if (!data?.watchId || !data.buildUuid || !data.threadUuid) {
      ...
      // Without this, a rejected payload leaves its dedupe marker holding off re-arms for the
      // full marker TTL (observed live as "duplicate watch skipped" after a dead watch).
      if (typeof data?.markerKey === 'string' && data.markerKey) {
        await this.releaseMarker(data as AgentEnvironmentWatchJob);
      }
      return;
    }
    ...
    try {
      ...
      if (outcome === 'pending' && !expired) {
        // First observed activity: tell the thread the rebuild is underway (once per watch).
        let activityEventPosted = data.activityEventPosted === true;
        if (sawActivity && !activityEventPosted) {
          await this.postStateEvent(data, 'started', false);
          activityEventPosted = true;
        }
      ...
      // Failure outcomes collect fresh triage so the next turn opens with decisive evidence.
      await this.postStateEvent(data, messageOutcome, messageOutcome === 'failure');
      await this.releaseMarker(data);
      logger().info(
  ...
  private static async postStateEvent(
    ...
    if (!session) {
      logger().info(`EnvWatch: session missing threadUuid=${data.threadUuid} buildUuid=${data.buildUuid}`);
      return;
    }
    ...
    await EnvironmentStateService.postWatchStateEvent({
      session,
      thread,
      uuidSeed: `${data.watchId}:${outcome === 'started' ? 'activity' : 'final'}`,
      headline: buildEnvironmentWatchHeadline(outcome, data.reason),
      includeTriage,
  ...
  private static async releaseMarker(data: AgentEnvironmentWatchJob): Promise<void> {
    try {
      const markerKey =
        (typeof data.markerKey === 'string' && data.markerKey) ||
        environmentWatchDedupeKey(data.buildUuid, data.threadUuid);
      await RedisClient.getInstance().getConnection().del(markerKey);
    } catch (error) {
