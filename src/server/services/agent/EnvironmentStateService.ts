...
export function renderEnvironmentStateDelta(
  ...
  const primary = changes[0].replace(/^-\s*/, '');
  const summary = changes.length > 1 ? `${primary} (+${changes.length - 1} more)` : primary;
...
export default class EnvironmentStateService {
  ...
  static async ensureRunStartStateEvent({
    ...
    // Not namespace: freeform CHAT sessions have a workspace namespace but no build to report on.
