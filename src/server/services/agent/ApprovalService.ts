...
const ALWAYS_ALLOW_INELIGIBLE_CAPABILITIES = new Set<string>(['git_write']);
...
export default class ApprovalService {
  ...
  // SECURITY: the respond endpoint's 'never auto-approve git_write' contract, for allowlist chokepoints.
  static isToolKeyAlwaysAllowEligible(toolKey: string, toolMetadata?: AgentRuntimeToolMetadata[]): boolean {
    const capabilityKey = resolveApprovalCapabilityKey(toolKey, 'read', toolMetadata);
    return !ALWAYS_ALLOW_INELIGIBLE_CAPABILITIES.has(capabilityKey);
  }
  ...
  static async resolvePendingAction(
    ...
    const actionSeed = (await AgentPendingAction.query()
      ...
      .select('action.*', 'thread.uuid as threadUuid', 'run.uuid as runUuid')
      .first()) as (AgentPendingAction & { threadUuid?: string; runUuid?: string }) | undefined;

    if (!actionSeed) {
      throw new Error('Pending action not found');
    }

    // GitHub probes resolve before the row lock; 'approved' covers the duplicate-approve/requeue branch.
    let preResolvedGitWriteAuth: AgentRequestGitHubAuth | null = null;
    if (
      approved &&
      actionSeed.capabilityKey === 'git_write' &&
      (actionSeed.status === 'pending' || actionSeed.status === 'approved')
    ) {
