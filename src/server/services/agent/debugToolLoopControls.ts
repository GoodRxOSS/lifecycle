...
function exceedsRunInputTokenBudget(steps: UsageSteps, maxRunInputTokens: number): boolean {
  return cumulativeInputTokens(steps) >= maxRunInputTokens;
}
...
// Repair tools return agentContent text ("Error: …" on failure), a ToolResult ({ success }), or the
// ModelMessage envelope ({ type: 'json'|'text'|'error-*', value }). A failed mutation must NOT trip
// the one-mutation gate — the model should be able to retry a rejected commit.
function isSuccessfulMutationOutput(output: unknown): boolean {
  if (output == null) {
    return false;
  }
  ...
  const record = output as { type?: unknown; value?: unknown; success?: unknown; isError?: unknown };
  if (typeof record.type === 'string' && record.type.startsWith('error')) {
    return false;
  }
  ...
  if ('value' in record) {
    return isSuccessfulMutationOutput(record.value);
  }
  return true;
}

function repairMutationLandedInSteps(steps: ToolResultSteps, mutationToolKeys: ReadonlySet<string>): boolean {
  return steps.some((step) =>
    (step.toolResults || []).some(
      (result) => mutationToolKeys.has(result.toolName) && isSuccessfulMutationOutput(result.output)
    )
  );
}

// Approval pauses resume as a fresh stream whose `steps` omit the pre-pause mutation result; the
// committed change is still in the model's message history, so the gate must read from there too.
// ONLY the current run's tail counts: tool results from EARLIER runs also live in the replayed
// history (they persist as tool_call parts), and matching them would permanently lock every later
// repair run in the thread to read-only — the "read-only investigation session" bug. Everything
// after the last user-role message belongs to the current turn; prior runs' results always
// precede a later user prompt.
function repairMutationLandedInMessages(
  messages: ReadonlyArray<ModelMessage>,
  mutationToolKeys: ReadonlySet<string>
): boolean {
  const lastUserIndex = messages.reduce((last, message, index) => (message.role === 'user' ? index : last), -1);
  return messages.some((message, index) => {
    if (index <= lastUserIndex || message.role !== 'tool' || !Array.isArray(message.content)) {
      return false;
    }
    return message.content.some(
      (part) =>
        part?.type === 'tool-result' &&
        mutationToolKeys.has(part.toolName) &&
        isSuccessfulMutationOutput((part as { output?: unknown }).output)
    );
  });
}
...
function stopWhenInputTokenBudgetExhausted(maxRunInputTokens: number): AgentStopCondition {
  return ({ steps }) => exceedsRunInputTokenBudget(steps.slice(0, -1), maxRunInputTokens);
}

function withInputTokenBudget(
  maxRunInputTokens: number,
  prepareStep?: AgentPrepareStepFunction
): AgentPrepareStepFunction {
  return (options) => {
    const inner = prepareStep?.(options);
    if (!exceedsRunInputTokenBudget(options.steps, maxRunInputTokens)) {
      return inner;
    }
...
export function resolveDebugToolLoopControls({
  ...
  const stopWhen = [isStepCount(effectiveMaxIterations), stopWhenInputTokenBudgetExhausted(maxRunInputTokens)];
  ...
  if (!intent) {
    ...
    if (!isBuildContext && (!isFreeform || workspaceReady)) {
      return { stopWhen, effectiveMaxIterations, prepareStep: withInputTokenBudget(maxRunInputTokens) };
    }
    ...
    if (!isFreeform) {
      return {
        ...
        prepareStep: withInputTokenBudget(maxRunInputTokens),
      };
    }
    ...
    return {
      ...
      stopWhen,
      effectiveMaxIterations,
      prepareStep: withInputTokenBudget(maxRunInputTokens, ({ steps, messages = [], initialInstructions }) => {
  ...
  // One mutation per repair run: the first committed change already triggers a rebuild (webhook or
  // redeploy), so further mutations in the same run race that rebuild against a drifting world —
  // the user watched commit #2 and a redundant trigger_redeploy stack three rebuilds. After a
  // successful mutation the loop narrows to read-only tools; the rebuild watch reports the outcome.
  const mutationToolKeys: ReadonlySet<string> = new Set(
    toolMetadata
      .filter((metadata) => registeredToolKeys.has(metadata.toolKey))
      .filter((metadata) => isRepairRuntimeTool(metadata) && !isReadOnlyRuntimeTool(metadata))
      .map((metadata) => metadata.toolKey)
  );
  const postMutationActiveTools = activeTools.filter((toolKey) => !mutationToolKeys.has(toolKey));
  ...
  return {
    ...
    prepareStep: withInputTokenBudget(maxRunInputTokens, ({ stepNumber, steps, messages = [] }) => {
      if (
        intent === 'repair' &&
        !mutationLanded &&
        mutationToolKeys.size > 0 &&
        (repairMutationLandedInSteps(steps, mutationToolKeys) ||
          repairMutationLandedInMessages(messages, mutationToolKeys))
      ) {
        mutationLanded = true;
      }
      const stepActiveTools = mutationLanded ? postMutationActiveTools : activeTools;
      return stepNumber >= toolStepLimit
        ? { activeTools: stepActiveTools, toolChoice: 'none' as const }
