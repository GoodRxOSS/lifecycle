...
// A cheap truth probe for the request_workspace short-circuit: `workspaceStatus` is a claim, the
// gateway answering is the fact. Discovery success also warms the tool-discovery cache.
const READY_VERIFY_TIMEOUTS: SessionWorkspaceGatewayTimeouts = {
  discoveryTimeoutMs: 5000,
  executionTimeoutMs: 5000,
};

async function isWorkspaceGatewayLive(session: AgentSession): Promise<boolean> {
  try {
    return Boolean(await resolveSessionWorkspaceGatewayServer(session, READY_VERIFY_TIMEOUTS));
  } catch {
    return false;
  }
}
...
async function waitForChatWorkspaceRequest({
  ...
  // Two attempts: when a "ready" workspace fails gateway verification, settle the confirmed loss and
  // re-ensure — the second pass provisions fresh (or resumes) instead of returning a ready lie.
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (isChatWorkspaceRuntimeReady(runtimeSession)) {
        if (await isWorkspaceGatewayLive(runtimeSession)) {
          return {
        ...
        const settled = await AgentSessionService.reconcileLostChatWorkspaceRuntime(runtimeSession.uuid, {
          allowedActiveRunUuid,
        });
        ...
        return {
          ...
          workspaceStatus: (settled ?? runtimeSession).workspaceStatus,
          message:
            'Workspace is marked ready but its runtime is unreachable. Request the workspace again to re-provision.',
        };
      }
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const latestSession = await loadLatestSession(session.uuid);
    if (isChatWorkspaceRuntimeReady(latestSession) && (await isWorkspaceGatewayLive(latestSession))) {
      return {
        status: 'ready' as const,
...
export function registerChatRequestWorkspaceTool({
  ...
  tools[toolKey] = toAiDynamicTool({
    description:
      ...
      'Idempotent: returns immediately when a workspace is already ready, and recovers a lost or hibernated workspace when workspace tools report it unavailable.',
