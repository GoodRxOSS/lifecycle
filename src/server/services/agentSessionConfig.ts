...
import {
  ...
  DEFAULT_AGENT_SESSION_MAX_RUN_INPUT_TOKENS,
...
import { LIFECYCLE_DIAGNOSTIC_TOOL_MANIFEST } from './agent/diagnosticTools';
...
function normalizeControlPlaneConfig(value: unknown): AgentSessionControlPlaneConfigValue {
  ...
  return {
    ...
    maxRunInputTokens: normalizePositiveInteger((value as { maxRunInputTokens?: unknown }).maxRunInputTokens),
...
function hasConfigValues(config: Partial<AgentSessionControlPlaneConfigValue>): boolean {
  return Boolean(
    normalizeOptionalString(config.systemPrompt) ||
      ...
      normalizePositiveInteger(config.maxRunInputTokens) ||
...
export default class AgentSessionConfigService extends BaseService {
  ...
  async getEffectiveConfig(repoFullName?: string): Promise<EffectiveAgentSessionControlPlaneConfig> {
    ...
    return {
      ...
      maxIterations:
        ...
        DEFAULT_AGENT_SESSION_MAX_ITERATIONS,
      maxRunInputTokens:
        normalizePositiveInteger(repoConfig?.maxRunInputTokens) ||
        normalizePositiveInteger(globalConfig.maxRunInputTokens) ||
        DEFAULT_AGENT_SESSION_MAX_RUN_INPUT_TOKENS,
  ...
  async listToolInventory(scope: string): Promise<AgentSessionToolInventoryEntry[]> {
    ...
    // Debug's diagnostic/repair tools are rule targets too — without these entries the admin
    // panel could not set per-tool modes (allow/require/deny) for them at all.
    for (const tool of LIFECYCLE_DIAGNOSTIC_TOOL_MANIFEST) {
      appendEntry({
