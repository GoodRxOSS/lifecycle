...
import { combineAgentSessionAppendSystemPrompt } from 'server/lib/agentSession/systemPrompt';
import {
...
export default class AgentSessionService {
  ...
  // Session-stable lines only. Volatile environment state is appended to the conversation as
  // environment_state events (EnvironmentStateService) so the system prompt stays byte-stable
  // across runs and provider prompt caches keep hitting.
  static async getSessionAppendSystemPrompt(
    ...
    const lines: string[] = [];
    if (session.skillPlan?.skills?.length) {
      lines.push(
    ...
    const hasReadyWorkspace =
      session.workspaceStatus === AgentWorkspaceStatus.READY && Boolean(session.namespace) && Boolean(session.podName);
    ...
    if (toolLines.length > 0) {
      lines.push('- equipped tools:', ...toolLines.map((line) => `  ${line}`));
    }

    return combineAgentSessionAppendSystemPrompt(resolvedConfiguredPrompt, lines.length ? lines.join('\n') : undefined);
  }
