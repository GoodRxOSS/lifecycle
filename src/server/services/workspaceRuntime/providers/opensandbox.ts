...
export interface OpenSandboxRuntimeProviderState {
  ...
  gatewayToken?: string;
  /** Plan-derived primary repo mount path, persisted so a gateway restarted on resume (no plan) re-exports the correct value. */
...
export class OpenSandboxRuntimeService implements RemoteWorkspaceRuntimeProvider {
  ...
  async provision(ctx: RemoteProvisionContext): Promise<RemoteRuntimeHandle> {
    ...
    let state: OpenSandboxRuntimeProviderState = {
      ...
      primaryRepoPath: this.gatewayRuntimeEnv(plan).LIFECYCLE_SESSION_PRIMARY_REPO_PATH,
    };
  ...
  private gatewayRuntimeEnv(plan?: WorkspaceRuntimePlan, primaryRepoPath?: string): Record<string, string> {
    // On a plan-less gateway restart (resume), fall back to the persisted primary repo path instead of
    // the workspace-root default, which would clobber the correct mount for multi-repo sessions.
    return buildSessionRuntimeEnv(
      plan,
      this.config.gatewayPort,
      !plan && primaryRepoPath ? { LIFECYCLE_SESSION_PRIMARY_REPO_PATH: primaryRepoPath } : undefined
    );
  }
  ...
  private async ensureRuntimeEndpoints(
    ...
    if (!(await isHttpReady(joinUrl(gatewayUrl, '/health'), gatewayHeaders, 1000))) {
      const gatewayCommand = await this.runCommand(
        state,
        this.gatewayCommand(opts.plan, opts.gatewayToken, state.primaryRepoPath),
        {
          background: true,
          workingDirectory: SESSION_WORKSPACE_ROOT,
        }
  ...
  private gatewayCommand(plan?: WorkspaceRuntimePlan, gatewayToken?: string, primaryRepoPath?: string): string {
    const exports = Object.entries({
      ...(gatewayToken ? { [LIFECYCLE_GATEWAY_TOKEN_ENV]: gatewayToken } : {}),
      ...this.gatewayRuntimeEnv(plan, primaryRepoPath),
