...
import { WorkspaceRuntimeSecurityError, type RemoteProvisionContext, type WorkspaceRuntimeEndpoint } from '../types';
...
/** Parses a persisted preview-exposure row ({url, headers?}) into a proxy endpoint. */
export function parsePersistedExposureEndpoint(exposureState: unknown): WorkspaceRuntimeEndpoint | null {
  if (!isRecord(exposureState)) {
    return null;
  }
  const url = readString(exposureState.url);
  if (!url) {
    return null;
  }
