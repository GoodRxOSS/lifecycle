...
export interface ModalRuntimeProviderState {
  ...
  /** Prior snapshot, retained until the newer one is durably persisted, then GC'd on the next cycle. */
  previousSnapshotImageId?: string;
  checkpointAt?: string;
  gatewayUrl?: string;
...
export class ModalRuntimeService implements RemoteWorkspaceRuntimeProvider {
  ...
  async suspend(state: unknown, _opts: { retainForMs: number }): Promise<RemoteRuntimeHandle> {
    ...
    return this.withClient(async (_sdk, client) => {
      ...
      await sb.terminate();
      // GC only the prior-cycle snapshot: it was superseded by parsed.snapshotImageId, which is durably
      // persisted, so deleting it now cannot strand the DB. The current snapshot is kept (carried forward
      // as previousSnapshotImageId) until the caller persists this new one, so a persist failure or a
      // superseded lifecycle claim can still resume from it.
      await this.deleteSnapshotIfReplaced(client, parsed.previousSnapshotImageId, snapshot.imageId);
      ...
      return {
        ...
        providerState: {
          ...
          previousSnapshotImageId: parsed.snapshotImageId ?? null,
  ...
  async checkpoint(state: unknown): Promise<RemoteRuntimeHandle> {
    ...
    return this.withClient(async (_sdk, client) => {
      ...
      const snapshot = await sb.snapshotFilesystem(SNAPSHOT_TIMEOUT_MS);
      // GC the prior-cycle snapshot only (superseded by the durably-persisted parsed.snapshotImageId);
      // keep the current one until the caller persists this checkpoint.
      await this.deleteSnapshotIfReplaced(client, parsed.previousSnapshotImageId, snapshot.imageId);

      return this.toHandle({
        ...parsed,
        snapshotImageId: snapshot.imageId,
        previousSnapshotImageId: parsed.snapshotImageId,
        checkpointAt: new Date().toISOString(),
      });
    });
  }
  ...
  async destroy(state: unknown): Promise<void> {
    ...
    await this.withClient(async (_sdk, client) => {
      ...
      if (parsed.previousSnapshotImageId && parsed.previousSnapshotImageId !== parsed.snapshotImageId) {
        await client.images.delete(parsed.previousSnapshotImageId).catch(() => {});
      }
    });
  }
