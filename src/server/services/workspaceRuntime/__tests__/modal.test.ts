...
describe('suspend and checkpoint', () => {
  it('snapshots the filesystem before terminating and defers GC of the superseded snapshot', async () => {
    ...
    // The current snapshot must survive until the caller durably persists the new one, so a persist
    // failure or a superseded lifecycle claim can still resume; it is carried forward for later GC.
  ...
  it('GCs the prior-cycle snapshot on the next suspend, never the durable current one', async () => {
    const sb = fakeSandbox('sb-1', {
      snapshotFilesystem: jest.fn().mockResolvedValue({ imageId: 'im-snap' }),
    });
    modalMocks.sandboxesFromId.mockResolvedValue(sb);
    ...
    const handle = await service.suspend(
      { ...runningState, previousSnapshotImageId: 'im-older' },
      { retainForMs: 120_000 }
    );

    expect(modalMocks.imagesDelete).toHaveBeenCalledWith('im-older');
    expect(modalMocks.imagesDelete).not.toHaveBeenCalledWith('im-old');
    expect(handle?.providerState).toMatchObject({ snapshotImageId: 'im-snap', previousSnapshotImageId: 'im-old' });
  });
  ...
  it('checkpoints without terminating and keeps the sandbox handle', async () => {
    ...
    // Current snapshot deferred for GC, not deleted before the checkpoint is persisted.
    expect(modalMocks.imagesDelete).not.toHaveBeenCalled();
    expect(handle?.providerState).toMatchObject({
      sandboxId: 'sb-1',
      snapshotImageId: 'im-ckpt',
      previousSnapshotImageId: 'im-old',
    });
  });
